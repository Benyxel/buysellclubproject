...
export default function CorporateAgentManagement() {
  ...
  const loadAgents = async () => {
    try {
      ...
      // Filter for corporate agents (assuming agent_type field or similar)
      const corporateAgents = allAgents.filter(
        (agent) => agent.agent_type === "corporate" || agent.agent_category === "corporate"
      );
      setAgents(corporateAgents);
    } catch (err) {
      console.error("Failed to load corporate agents", err);
      const status = err.response?.status;
      if (status && status >= 400) {
        const errorMsg = err.response?.data?.detail || err.response?.data?.error || err.message || "Failed to load corporate agents";
        toast.error(errorMsg, { toastId: "load-corporate-agents-error" });
      }
      setAgents([]);
    } finally {
      setLoading(false);
  ...
  const handleAssignAgent = async () => {
    ...
    try {
      setAssigning(true);
      // Assign as corporate agent
      const response = await API.post("/buysellapi/admin/agents/", {
        user_id: userToAssign.id,
        agent_type: "corporate", // Specify agent type
      });
      ...
      console.error("Failed to assign corporate agent", err);
      let errorMsg = "Failed to assign corporate agent";
  ...
  const handleRemoveAgent = async () => {
    ...
    try {
      ...
      console.error("Failed to remove corporate agent", err);
      const errorMsg = err.response?.data?.detail || 
                      err.response?.data?.error || 
                      "Failed to remove corporate agent";
      toast.error(errorMsg);
    } finally {
      setLoading(false);
    }
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Manage corporate agents who bring goods to the platform
        </p>
      </div>
      ...
      {/* Corporate Agents List */}
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
          <FaUserTag className="text-blue-600" />
          Corporate Agents ({filteredAgents.length})
        </h3>
        {loading && filteredAgents.length === 0 ? (
          <div className="text-center py-8">Loading corporate agents...</div>
        ) : filteredAgents.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            No corporate agents found
          </div>
        ) : (
      ...
      <ConfirmModal
        ...
        title="Assign Corporate Agent"
        message={`Are you sure you want to assign ${userToAssign?.username || userToAssign?.full_name} as a corporate agent?`}
      ...
      <ConfirmModal
        ...
        title="Remove Corporate Agent"
        message={`Are you sure you want to remove ${agentToRemove?.user?.username || agentToRemove?.username} as a corporate agent?`}
